...
def main() -> None:
    ...
    if args.json:
        # Suppress stdout chatter when producing machine-readable output
        ...
        if args.output:
            ...
            print(
                f"[html_importer] JSON written to {args.output}",
                file=sys.stderr,
                flush=True,
            )
        else:
        ...
        print(
            f"[html_importer] Processing {len(html_files)} file(s) from: {target}",
            flush=True,
        )
        print("=" * 80, flush=True)
        ...
        for filepath in html_files:
            ...
            if metadata.title or metadata.conversation_id or metadata.model:
                ...
                if metadata.conversation_id:
                    print(f"   Conv ID:  {metadata.conversation_id}", flush=True)
                if metadata.model:
                    print(f"   Model:    {metadata.model}", flush=True)
                if metadata.create_time:
                    print(f"   Created:  {metadata.create_time}", flush=True)
                if metadata.update_time:
                    print(f"   Updated:  {metadata.update_time}", flush=True)
                print(flush=True)
        ...
        print("=" * 80, flush=True)
        print(
            f"[html_importer] Done. Total: {total_messages} message(s) "
            f"from {len(html_files)} file(s).",
            flush=True,
        )
