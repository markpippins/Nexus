/**
 * tools-aggregator — command registry read-model (aggregator-owned).
 *
 * The single owner of `mcp.command_registry` access. Folded in from
 * slash-command-mcp (D-2026-08-16-002): the registry is a read-model owned
 * by the aggregator, so slash clients never touch PostgreSQL directly —
 * they ask the aggregator (REST `/commands/*` or the native command_* tools).
 *
 * All lookups are read-only; the registry is refreshed out-of-band by the
 * mcp-registry-seeder, never by this server.
 */
...
/** Search commands across all services by command-name prefix. */
export async function searchCommands(
  ...
  await client.connect();
  try {
    const { rows } = await client.query<{ command: string; service: string }>(
