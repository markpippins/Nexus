...
> **Design:** `semantics-db.md` · **Migrations:** `semantics.sql` + V055–V059 (see chain below)
...
|-------|---------|---------------------|
| `owning_subsystem` | The fleet subsystems (stable `smallint` lookup keys) | `id` PK, `name` UNIQUE-active, `description`, `expired_at` |
...
| `concept_relationship` | Legal pipeline shape between classes | `from/to_concept_id` FKs, `relationship_type`, `path` (`green`/`red`), `effective_at` |

**Integrity:** 14 FKs (all enforced — this is a low-write curated graph) +
...
**Shared vocabularies (text columns):**

| Column | Allowed values |
|--------|----------------|
| `representation_relationship.relationship_type` | `equivalent`, `derived`, `partial`, `legacy`, `supersedes`, `projects` |
| `consumer_operation.operation` | `reads`, `writes`, `observes`, `emits`, `projects`, `owns` |
| `snapshot_observation.lifecycle_state` | `active`, `deprecated`, `migrating`, `legacy-frozen`, `expired` |
| `concept_relationship.relationship_type` | `produces`, `spawns`, `member_of`, `transforms_into`, `basis_of`, `provenance_of`, `parent_of` |
...
|--------|-------|---------|
| `add_<table>(...)` | 11 | Plain `INSERT … RETURNING *`; uuid auto-generated via `COALESCE(p_id, gen_random_uuid())`; `p_*` params all defaulted (NULL) |
| `soft_delete_<table>(p_id)` | 11 | `SET expired_at = NOW() WHERE expired_at IS NULL`; returns `integer` rows updated (0 = already gone — idempotent) |
| `update_<table>(p_id, …)` | 11 | **Append-only replace:** expire the active row, insert a fresh version with a new uuid, atomically; raises `no active row with id …` if nothing active. `update_owning_subsystem` takes `p_new_id` (smallint key is caller-supplied) |
...
|--------|------|----------|
| `owning_subsystem` | 16 | nebula, conduit, assembly, tackle, cascade, harness, wind, harvest, peb, knowledge, vision, terrain, timeclock, address-tts, bitemporal-api, semantics |
...
cat nexus/sql/V059__semantics_seed_lookup.sql \
...
psql -h strontium -p 5432 -U pguser -d nexus -v ON_ERROR_STOP=1 \
  -f nexus/sql/V059__semantics_seed_lookup.sql
```
