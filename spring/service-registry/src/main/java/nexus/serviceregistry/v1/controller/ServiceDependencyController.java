...
public class ServiceDependencyController {
    ...
    @GetMapping
    public ResponseEntity<com.angrysurfer.nexus.dto.PagedResponse<nexus.serviceregistry.v1.entity.ServiceDependency>> getAllDependencies(org.springframework.data.domain.Pageable pageable) {
        log.info("Fetching all service dependencies from console");
        List<nexus.serviceregistry.v1.entity.ServiceDependency> list = client.getServiceDependencies();
        int start = (int) pageable.getOffset();
        int end = Math.min((start + pageable.getPageSize()), list.size());
        org.springframework.data.domain.Page<nexus.serviceregistry.v1.entity.ServiceDependency> page = new org.springframework.data.domain.PageImpl<>(
