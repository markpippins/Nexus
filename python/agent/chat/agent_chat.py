...
# ── Reuse from executor_cloud ─────────────────────────────────────
from executor_cloud import (
    OPENCODE_BIN,
...
# ── DB-backed AI config resolution ──────────────────────────────

_RoleConfigCache: dict | None = None  # role → config dict, cached for 60s
_RoleConfigCacheTime = 0
_ROLE_CONFIG_TTL = 60  # seconds — cache DB lookups for 1 minute


def _resolve_role_config(role: str) -> dict | None:
    """Look up the AI config for a role from the conduit DB.

    Returns a dict with keys: model_identifier, provider_type, harness_name,
    invocation_semantics (parsed JSON), api_key, endpoint_url,
    or None if not configured.
    Caches results briefly since these rarely change.
    """
    ...
    try:
        ...
        for role_name in VALID_ROLES:
            cfg = db.get_role_model_config(role_name)
            # get_role_model_config returns {'harness': ..., 'model': ...};
            # for chat we need the full row from the DB, not just harness+model.
            # Fall back to DB query if we need provider info.

        # Query all role configs at once to populate cache
        with db._get_connection() as conn:
            cursor = conn.execute(
                """SELECT rc.role,
                   ...
                   FROM vector.role_config rc
                   JOIN vector.models m   ON rc.model_id    = m.id
                   JOIN vector.providers p ON rc.provider_id = p.id
                   JOIN vector.harnesses h ON rc.harness_id  = h.id"""
            )
        ...
        for row in rows:
            ...
            # v098: Also fetch fallback models with per-model provider/harness
            fallback = db.get_fallback_models(row["role"])
        ...
        _RoleConfigCache = cache
        _RoleConfigCacheTime = now
        return cache.get(role)
    except Exception as e:
        print(f"[ai-config] DB lookup failed: {e}", file=sys.stderr)
        return None
