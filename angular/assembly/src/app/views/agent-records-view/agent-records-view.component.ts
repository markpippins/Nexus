...
@Component({
  selector: 'app-agent-records-view',
  standalone: true,
  ...
  templateUrl: './agent-records-view.component.html',
})
export class AgentRecordsViewComponent implements OnInit {
  ...
  private load() {
    ...
    this.dataService.getAgentRecords().subscribe({
      ...
      error: err => {
        this.error.set(err.message || 'Failed to load agent records');
        this.loading.set(false);
      }
    });
  }
