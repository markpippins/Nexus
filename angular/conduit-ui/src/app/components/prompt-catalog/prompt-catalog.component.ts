...
@Component({
  ...
  standalone: true,
  imports: [NgFor, NgClass, FormsModule, LoadingSpinnerComponent, ErrorBannerComponent, EmptyStateComponent],
  template: `<div class="dashboard">
    ...
    <div class="search-bar"><input class="search" [(ngModel)]="search" (ngModelChange)="onSearch($event)" placeholder="Search prompts...">@if (search || activeLoc()!=='all' || responseFilter()!=='all') {<button class="btn-clear" (click)="clearAll()">✕ Clear</button>}</div>
    ...
    @if (!loading() && !error()) {
      <div class="results">
        <div class="row" *ngFor="let p of results()" (click)="expanded()===p.path?expanded.set(null):expanded.set(p.path)">
          ...
          @if (expanded()===p.path) {<div class="detail"><div class="d-label">Plan Lineage</div><div class="d-value">{{p.invocationOrder.join(' → ') || 'none'}}</div><div class="d-label">Prompt</div><div class="d-value">{{p.summary}}</div>@if (p.response) {<div class="d-label">Response</div><div class="d-value response-text">{{p.response}}</div>}</div>}
        </div>
        @if (results().length===0) {<app-empty-state icon="📝" title="No prompts found" description="Try adjusting your search or filter." />}
      </div>
    }
  </div>`,
