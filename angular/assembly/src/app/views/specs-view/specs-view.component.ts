...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, PageHeaderComponent, TableSkeletonComponent, EmptyStateComponent, ErrorStateComponent, RaiseQuestionComponent, PaginationComponent],
...
export class SpecsViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<SpecItem>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getSpecs(this.currentPage(), this.pageSize).subscribe({
      next: res => {
