...
class DedupeCache:
    ...
    # ── Directory-level dedupe (Stage 3) ───────────────────────────────────
    # Keyed by fs:dirc:{path}; stores the directory's signature computed from
    # its own stat + direct-child aggregate stats. When a subtree is unchanged,
    # the scanner skips descending into it, turning O(files) per-file Redis
    # probes (fs:cache) into O(dirs) directory probes per epoch.
