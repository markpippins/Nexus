...
DECLARE
    ...
    v_right_id  uuid;
BEGIN
    SELECT kind, operator, literal_value, attribute_id INTO v_kind, v_operator, v_literal, v_attr_id
...
-- extend compile_exists_chain: the chain's terminal child may now be a
-- relationship_ref (keep nesting EXISTS) OR a real condition (attribute_ref/
-- literal/operator) to AND in as the leaf predicate, instead of bare TRUE.
...
BEGIN
    ...
    IF v_child_id IS NOT NULL THEN
        SELECT kind INTO v_child_kind FROM resolution.expression WHERE id = v_child_id;
        IF v_child_kind = 'relationship_ref' THEN
