...
| `src/db.ts`                | PostgreSQL schema (all migrations), `plan_status` view, tickets CRUD, AI config registry (v083), full CRUD for plans/receipts/sessions/circuit_breaker/tickets. |
...
```
nexus/.conduit-data/
├── (PostgreSQL — nexus database, conduit schema)
...
- **vector.providers, vector.harnesses, vector.models, vector.role_config** — AI config registry (v083)
...
# Clean stale sessions (via Docker Postgres)
docker exec nexus-postgres psql -U pguser -d nexus \
  -c "UPDATE sessions SET is_running=0, end_iso=NOW() WHERE is_running=1;"
...
```bash
docker exec nexus-postgres psql -U pguser -d nexus \
  -c "SELECT id, title FROM plan_status WHERE derived_status IS NULL AND deleted=0;"
```
