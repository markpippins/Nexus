...
class GeminiParser(BaseParser):
    ...
    Detection heuristic: exported markdown contains "**Gemini" speaker
    labels or the source filename contains "gemini".
    """
    ...
    def can_handle(self, doc, source_path: Path) -> bool:
        ...
        has_gemini_label = any("**Gemini" in line for line in md.split('\n'))
        filename_lower = source_path.name.lower()
        is_gemini_filename = 'gemini' in filename_lower
        return has_gemini_label or is_gemini_filename
    ...
    def parse(self, doc, source_path: Path, metadata: ConversationMetadata) -> list[NormalizedMessage]:
        """Parse Gemini chat exports from DoclingDocument markdown output."""
        md = doc.export_to_markdown()
            ...
            if matched_speaker is not None:
                if current_speaker and current_text_parts:
                    ...
                    messages.append(NormalizedMessage(
                        message_id=f'gemini-msg-{len(messages)}',
