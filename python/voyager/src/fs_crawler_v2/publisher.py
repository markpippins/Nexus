...
class Publisher:
    def __init__(self, nats_url=None, origin_layer=None):
        self.nc = None
        self.js = None  # JetStream context (persistent publish)
    ...
    async def connect(self):
        if self.nats_url:
            try:
                ...
                # ── JetStream context for persistent events ──
                try:
    ...
    async def publish(self, subject: str, event: CanonicalEnvelope):
        """Publishes a validated event to NATS or logs it if NATS is unavailable."""
        # Enforce SCCM contracts
        ContractValidator.validate_emission(event, publisher_layer=self.origin_layer)

        # Use to_dict() for serialization (unified envelope standard)
        ...
        if self.nc:
            try:
                if self.js is not None:
                    ...
                    await self.nc.publish(subject, json.dumps(event_dict).encode())
                    await self.nc.flush()  # force buffer → network (prevents silent loss on exit)
            except Exception as e:
    ...
    def scoped(self, layer: str):
        """Returns a new Publisher sharing the same connection and JetStream context."""
