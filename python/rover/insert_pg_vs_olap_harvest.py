#!/usr/bin/env python3
"""Insert Postgres vs OLAP for Analytics harvest into DB."""
import json
import subprocess
...
def main():
    with open("/tmp/pg_vs_olap_harvest.json") as f:
        data = json.load(f)
    candidates = data["agenda_items"]
    total = len(candidates)
    print(f"Total candidates: {total}")

    md_path = "/tmp/harvest_chunks/pg_vs_olap/Postgres_vs_OLAP_for_Analytics_full.md"
    with open(md_path) as f:
        source_text = f.read()
    print(f"Source text: {len(source_text)} chars")

    source_path = "chats/Postgres vs OLAP for Analytics.html"
    source_filename = "Postgres vs OLAP for Analytics.html"
    model = "opencode/big-pickle"
    tags = ["opencode", "inference-engine", "architect", "harvest", "postgres_vs_olap"]
    ...
    if out:
        ...
        print("✅ Postgres vs OLAP harvest inserted")
    else:
