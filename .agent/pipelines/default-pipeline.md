...
2. **`requirements-capture`**: Transformation of the raw prompt into a structured internal representation.
3. **`resource-binding`**: Mapping of the structured intent to specific project resources (files/directories).
4. **`decompose-task`**: Breaking down the goal into atomic units of work.
5. **`work-request-emission`**: Generation of lifecycle-aware `WorkRequest` objects (v1, v2, etc.).
6. **`archive-implementation`**: Snapshotting the planning state and materializing the `WORK_TO_DATE.md` projection.
