...
> **Companion document:** See [`nexus_irl_taxonomy.md`](./nexus_irl_taxonomy.md) for the IRL-side probabilistic interaction archetypes (8 types). This document defines the IR-side deterministic archetypes (9 types, closed contract). The IRL classifies intent probabilistically; this taxonomy governs how the deterministic structural graph is permitted to evolve in response.

> **Related:** [`AUTHORITY_GRAPH_IR.md`](./AUTHORITY_GRAPH_IR.md) defines the Append-Only Object Registry (IR) that these archetypes govern. [`VALIDATOR_SPEC.md`](./VALIDATOR_SPEC.md) defines the AEI (Authority Graph) validation dimensions that enforce these constraints at compile time.
