...
|---|---|
| [`messagebox-core-architecture.md`](./messagebox-core-architecture.md) | Core contract — this taxonomy defines the semantic roles that MessageBox carries |
| [`transport-abstraction-spec.md`](./transport-abstraction-spec.md) | How these semantic subjects map to NATS/Redis transport subjects |
| [`steward-spec.md`](./steward-spec.md) | Steward consumes `proposal.*` and emits `kg.mutation.*` using this taxonomy |

> **Related specs:** [`EVENT_GRAMMAR.md`](../EVENT_GRAMMAR.md) defines a parallel event type taxonomy for the CER event system. [`CER_SPEC.md`](../CER_SPEC.md) defines the Canonical Event Record format.
