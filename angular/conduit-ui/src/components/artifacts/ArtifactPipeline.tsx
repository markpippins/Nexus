    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1600px] mx-auto text-zinc-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-900/90 border border-zinc-800 rounded-lg p-4">
        ...
        <button
          ...
          className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-mono text-sm font-bold rounded flex items-center gap-1.5 transition-all shadow-[0_0_12px_rgba(99,102,241,0.3)] self-start md:self-auto"
        >
      ...
      <div className="flex items-center gap-2 border-b border-zinc-800 pb-2 overflow-x-auto">
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm font-mono font-semibold transition-all shrink-0 ${
            ...
            >
              ...
              <div className="pt-3 border-t border-zinc-800/80 flex items-center justify-between gap-2">
                <button
                  ...
                  className="px-2.5 py-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-sm font-mono rounded border border-zinc-700 flex items-center gap-1 transition-colors"
                >
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-indigo-950/80 hover:bg-indigo-900/80 text-indigo-300 border border-indigo-600/50 text-sm font-mono rounded flex items-center gap-1 transition-colors"
                >
            ...
            {candidates.map((cand) => (
              ...
              >
                <div className="space-y-1 max-w-3xl">
                  ...
                  <p className="text-sm text-zinc-400">{cand.description}</p>
                </div>

                <div className="shrink-0">
                  {cand.status === 'converted_to_intent' ? (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-950 text-emerald-300 border border-emerald-700 text-sm font-mono rounded font-semibold">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-zinc-950 font-mono text-sm font-bold rounded flex items-center gap-1.5 transition-all shadow-md"
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {intents.map((intent) => (
            ...
            >
              ...
              <p className="text-sm text-zinc-400">{intent.intentScope}</p>

              <div className="bg-zinc-950 p-2.5 rounded border border-zinc-800/80 text-sm font-mono">
              ...
              <div className="pt-2 flex justify-end">
                {intent.status === 'promoted_to_requirement' ? (
                  <span className="text-sm font-mono text-emerald-400 bg-emerald-950 px-2.5 py-1 rounded border border-emerald-800">
                    ✓ Promoted to Requirement
                  </span>
                ) : (
                  <button
                    onClick={() => onPromoteIntent(intent.id)}
                    className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-zinc-950 font-mono text-sm font-bold rounded flex items-center gap-1.5 transition-all"
                  >
          ...
          {requirements.map((req) => (
            ...
            >
              ...
              <div className="space-y-1">
                <span className="text-sm font-mono text-zinc-400">Acceptance Criteria:</span>
                <ul className="space-y-1 pl-4 list-disc text-sm text-zinc-300">
              ...
              <div className="pt-2 flex justify-end">
                {req.status === 'canonicalized' ? (
                  <span className="text-sm font-mono text-purple-300 bg-purple-950 px-2.5 py-1 rounded border border-purple-800">
                    Canonicalized System Spec
                  </span>
                ) : (
                  <button
                    onClick={() => onCanonicalizeReq(req.id)}
                    className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-mono text-sm font-bold rounded flex items-center gap-1.5 transition-all"
                  >
        ...
        <div className="space-y-4">
          {specs.map((spec) => (
            ...
            >
              ...
              <p className="text-sm text-zinc-300 font-sans">{spec.architectureSummary}</p>
