-- ─────────────────────────────────────────────────────────────────────
-- tackle.prompts(engineer, opencode-persona, v2)
-- Supersedes v1 per the MAX(version) convention (no is_latest column).
-- Engineer intent, this session (2026-07-25):
...
VALUES (
    ...
    'opencode-persona',
    2,
    'Engineer (opencode persona) — unrestricted primary agent; full filesystem + shell access; post-conduit plan-routing disambiguation',
    $eng_persona_v2_body$
...
$eng_persona_v2_body$,
    ...
    ARRAY['opencode-persona','category-1','engineer','v2']::TEXT[]
)
...
-- Update the default engineer task (if any) to point at the new latest.
-- Currently no tasks row references engineer, so this is a no-op-safe
-- statement (the WHERE clause filters out naturally).
UPDATE tackle.tasks
    SET prompt_id = (SELECT id FROM tackle.prompts WHERE role='engineer' AND slug='opencode-persona' AND version = (SELECT MAX(version) FROM tackle.prompts WHERE role='engineer' AND slug='opencode-persona')),
        updated_at = NOW()
    WHERE role = 'engineer'
      AND prompt_id <> (SELECT id FROM tackle.prompts WHERE role='engineer' AND slug='opencode-persona' AND version = (SELECT MAX(version) FROM tackle.prompts WHERE role='engineer' AND slug='opencode-persona'));
