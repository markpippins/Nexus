...
--     ├─ harvest_candidates.harvest_id                      → 3,542 rows
--     │    ├─ requirements.candidate_id                     → 1 row
--     │    └─ intent_records.source_ref (candidate UUID)    → 398 rows
--     └─ conversation_snapshots.conversation_id → segments  → 23,108 rows
--
-- Counts verified live 2026-08-18.
-- =============================================================================
...
-- 4) Intent records traceable to corrupted sources (source_ref = candidate UUID)
SELECT ir.id AS intent_record_id, ir.title, ir.source_ref, hc.id AS candidate_id, h.source_path
FROM nebula.intent_records ir
JOIN nebula.harvest_candidates hc ON hc.id::text = ir.source_ref
JOIN nebula.harvests h ON h.id = hc.harvest_id
WHERE h.source_path LIKE 'chats/%';

-- 5) Segments traceable to corrupted sources (via conversation snapshots)
...
UNION ALL SELECT 'requirements', count(*) FROM nebula.requirements r
    JOIN nebula.harvest_candidates hc ON hc.id = r.candidate_id
    JOIN nebula.harvests h ON h.id = hc.harvest_id                   WHERE h.source_path LIKE 'chats/%'
UNION ALL SELECT 'intent_records', count(*) FROM nebula.intent_records ir
    JOIN nebula.harvest_candidates hc ON hc.id::text = ir.source_ref
    JOIN nebula.harvests h ON h.id = hc.harvest_id                   WHERE h.source_path LIKE 'chats/%'
