...
def write_event(evt_dict):
    ...
    Phase 1: Also enqueues for NATS publish via sidecar thread.
    This is a non-breaking addition — the file write is unchanged.
    """
    path = os.path.join(OUTPUT_DIR, f"{evt_dict['id']}.json")
    with open(path, "w") as f:
        json.dump(evt_dict, f, indent=2)

    # ── Phase 1: Enqueue for NATS publish (fire-and-forget) ──
    _enqueue_nats_if_available(evt_dict)

    return path


def _enqueue_nats_if_available(evt_dict, **kwargs):
    """Enqueue an event (wrapped in CanonicalEnvelope) for NATS publish.

    Phase 1.5: Passes causation metadata through to the envelope adapter.
    Silently skipped if nats_publisher is not deployed or NATS is unavailable.
    """
    try:
...
def run_dispatcher():
    ...
    # ── Phase 1: Start NATS sidecar for this subprocess run ──
    nats_url = os.getenv("NATS_URL")
    if nats_url:
        try:
            from nats_publisher import start_nats_sidecar
            start_nats_sidecar(nats_url)
            _log("NATS sidecar started for dispatcher")
        except ImportError:
            pass
    ...
    try:
        ...
        # ── Phase 1: Stop NATS sidecar (graceful drain + flush + close) ──
        if nats_url:
            try:
                from nats_publisher import stop_nats_sidecar
                stop_nats_sidecar()
                _log("NATS sidecar stopped for dispatcher")
            except ImportError:
                pass
