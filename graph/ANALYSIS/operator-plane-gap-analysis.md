...
> "Nexus WRP aspirational architecture (inactive). The active system is Conduit."
...
| `jvm/spring/topology-server/` | Profile config server (port 8084) | ✅ Working |
...
| **Backend** | file-service, search-service, file-system-server, image-server, Moleculer search | Same + service-registry, topology-server, all broker modules |
