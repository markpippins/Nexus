...
public class FrameworkLanguageController {
    ...
    @GetMapping
    public ResponseEntity<com.aibizarchitect.nexus.v1.dto.PagedResponse<FrameworkLanguage>> getAll(
        ...
        return ResponseEntity.ok(com.aibizarchitect.nexus.v1.spring.serviceregistry.dto.SpringPagedResponse.fromPage(languages));
    }
    ...
    public ResponseEntity<FrameworkLanguage> update(@PathVariable Long id, @RequestBody FrameworkLanguage details) {
        ...
        return repository.findById(id)
                .map(existing -> {
                    ...
                    existing.setCurrentVersion(details.getCurrentVersion());
                    existing.setLtsVersion(details.getLtsVersion());
