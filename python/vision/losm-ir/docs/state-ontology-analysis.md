...
The IR-level / simplified lifecycle. No DB binding.
...
### WorkStatus (`losm_store/models.py`) — 11 states

The operational pipeline position. DB-backed column (`work_requests.status`).
...
The relationship is **many-to-one from WorkStatus to WorkflowState**:
...
The compression is concentrated in the *planning phase* where 3 WorkStatus states
...
The transition table (`VALID_TRANSITIONS` in `transition.py`) uses WorkStatus
...
- **WorkflowState becomes a computed projection** — a function
  `work_status_to_phase(s: WorkStatus) -> WorkflowState` that compresses the 11
  operational states into the 9 lifecycle phases.

This means:
- `losm_ir.states.WorkflowState` stays in losm-ir but is no longer a "parallel
  truth" — it's a view on top of WorkStatus.
- `losm_store.models.WorkStatus` stays in losm-store (it's tied to SQLAlchemy),
...
Since WorkStatus is now the canonical lifecycle enum, and `validate_transition`
needs to live in losm-ir (per architectural recommendation), WorkStatus should
move to losm-ir alongside it.

This breaks the current circular tension:
- `losm_ir` defines `WorkStatus` + `validate_transition` → canonical truth
- `losm_store` imports `WorkStatus` from `losm_ir` → no longer defines its own enum
- `losm_shell` imports both from `losm_ir` → consistent

### Migration order

1. Add `work_status_to_phase()` function to `losm_ir`
2. Move `WorkStatus` enum to `losm_ir`
3. Move `validate_transition` to `losm_ir` (Plan 0026)
4. Deprecate direct use of `WorkflowState` — prefer `work_status_to_phase()`
...
From `losm-shell/src/losm_shell/lifecycle/transition.py`:
