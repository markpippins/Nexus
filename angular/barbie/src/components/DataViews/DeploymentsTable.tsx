...
export const DeploymentsTable: React.FC<DeploymentsTableProps> = ({
  ...
  // search) keeps the existing rows visible and swaps in the fresh data when
  // it arrives, so the table doesn't blank out on every poll. Context changes
  // (page/size/filter/search) and the initial mount still show the loading
  // row because the data context genuinely changed.
