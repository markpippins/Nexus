...
@Deprecated
public class BrokerControllerV0 {

    private static final Logger log = LoggerFactory.getLogger(BrokerControllerV0.class);

    private final Broker broker;
    private final AdminLoggingService adminLoggingService;

    public BrokerControllerV0(Broker broker, AdminLoggingService adminLoggingService) {
        this.broker = broker;
        this.adminLoggingService = adminLoggingService;
        log.info("BrokerControllerV0 initialized");
    }
    ...
    private String extractErrorMessage(ServiceResponse<?> response) {
        // Extract error message from response, assuming error details are stored in
        // response
