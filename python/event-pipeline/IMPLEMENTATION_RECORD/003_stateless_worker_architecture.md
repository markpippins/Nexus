...
The existing `event-pipeline` relies on a `Dispatcher` that uses `read_offset()` to determine what task to execute next, maintaining a local "shadow state." To become Nexus-compliant, the Pipeline must become a "Stateless Worker." It has no memory of past steps, no internal sequence lists, and no offset trackers.
