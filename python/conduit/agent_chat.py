...
Usage:
    python3 agent_chat.py              # listen on port 3101
    AGENT_CHAT_PORT=3102 python3 ...   # custom port
...
PROJECT_ROOT = os.environ.get(
    ...
    os.path.dirname(os.path.dirname(
        os.environ.get("PIPELINE_DB_PATH", "/home/codex/dev/nexus/.conduit-data/pipeline.db")
    )),
)

PIPELINE_DB_PATH = os.environ.get(
    "PIPELINE_DB_PATH", "/home/codex/dev/nexus/.conduit-data/pipeline.db"
)
...
VALID_ROLES = ["planner", "builder", "reviewer", "critic"]
    ...
    try:
        ...
        rows = conn.execute(
            """SELECT rc.role,
               ...
               JOIN ai_providers p ON rc.provider_id = p.id
        ...
        cache: dict = {}
        for row in rows:
            semantics = row["invocation_semantics"] or "{}"
            try:
...
class ChatHandler(BaseHTTPRequestHandler):
    ...
    def _do_get(self):
        ...
        if path == "/chat/health":
            self._send_json({"status": "ok", "port": 3101})
            return
...
def main():
    ...
    port = int(os.environ.get("AGENT_CHAT_PORT", "3101"))
