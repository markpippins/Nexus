...
### 2.1 Plan Files (`IMPLEMENTATION_PLANS/{pending,active,completed,blocked,proposed,planning}/*.md`)
...
- On plan creation/update, a `.md` artifact file is **still written** to a flat `IMPLEMENTATION_PLANS/` directory (no status subdirectories) — consumed by external processes (AI agents, tooling, human review). The file is regenerated from DB state each write and **never read back** as operational state.
...
- `nexus/typescript/conduit-mcp/src/watcher.ts` — `createPlan()` keeps writing `.md` artifact files to flat `IMPLEMENTATION_PLANS/`; `updatePlanMetadata()` regenerates `.md` from DB state (no more regex-based in-place editing) but no longer reads files for state
...
**Affected code:**
- `nexus/python/conduit/main.py` — keeps writing DCO JSON files (artifact output) but the DB write is the authoritative path
- `nexus/python/conduit/db_adapter.py` — `add_work_request()` unchanged (already DB-only)
...
**Affected code:**
- `nexus/python/conduit/main.py` — stop writing `.meta.txt` files
...
### 2.10 `index.ts` Startup Scan (IMPLEMENTATION_PLANS/ upsert loop)

**Current behavior:**
- `index.ts` lines 682-727 contain a **separate** scan of `IMPLEMENTATION_PLANS/` directories that upserts plan `.md` files into the DB on startup — independent of the plan-watcher
...
**Current behavior:**
- The planner AI agent writes `.md` plan files directly to `IMPLEMENTATION_PLANS/proposed/` or `planning/`
...
1. **Plan `.md` files** — written to `IMPLEMENTATION_PLANS/` on plan create/update. Consumed by AI agents and human review. Artifact only; DB is authoritative.
